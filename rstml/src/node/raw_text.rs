...
/// `source_text` method is not available in `quote!` context, or in context
/// where input is generated by another macro. In still can return default
/// formatting for `TokenStream`.
...
impl<C> Default for RawText<C> {
    fn default() -> Self {
        Self {
            ...
            context_span: Option::default(),
            #[cfg(feature = "rawtext-stable-hack-module")]
            recovered_text: Option::default(),
            _c: PhantomData,
        }
    }
}
...
impl<C> RawText<C> {
    ...
    pub fn join_spans(&self) -> Option<Span> {
        ...
        for tt in self.token_stream.clone() {
        ...
        C: CustomNode,
    {
        let spans: Vec<Span> = Some(open_tag_end)
            .into_iter()
            .chain(children.iter().map(syn::spanned::Spanned::span))
